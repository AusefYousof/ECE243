...
CHECK_INPUT:
                CMP      R5, #73
				BLEQ	 KEY_ISR //the key interrupt is being sent
...
KEY_ISR:       	
				...
				LDR		R0, =0xFF200000 //R3 gets general IO address 
				LDR		R2, [R0, #0x5C] //load button edge
				STR		R2, [R0, #0x5C] //clear interrupt
				...
				CMP		R2, #4
				BLEQ	HALF_RATE
				//do something depending on which key is pressed
				POP		{PC}
...
K0_OR_K3:		LDR		R1, =RUN
				...
				CMP		R0, #1
				SUBEQ	R2, R0, #1  //if run is already one we need to stop it
				STREQ	R2, [R1]
...
//doubling rate = dividing inital value by 2
DOUBLE_RATE:	LDR		R0, =0xFFFEC600 
				LDR		R1, [R0]
				LSR		R1, #1
				STR		R1, [R0]
...
//half rate = multiply initial value by 2
HALF_RATE:		LDR		R0, =0xFFFEC600 
				LDR		R1, [R0]
				MOV		R2, #2
				MUL		R1, R1, R2
				STR		R1, [R0]
...
PRIV_TIMER_ISR:
              	LDR		R0, =0xFFFEC600 
				LDR		R1, [R0,#0xC] //load f bit (should be 1 because it made interrupt)
				STR		R1, [R0, #0xC]
